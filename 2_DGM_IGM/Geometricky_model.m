...
disp('Poloha koncoveho efektoru na zakladě D-H par.')
X = [Tn(1,4);
     Tn(2,4);
     atan2(Tn(2,1),Tn(1,1))]
...
%% IGM
disp('Zpetne zjisteni zadanych D-H par.')
Th = IGM_here(ai,X)
...
%% validace IGM a DGM
disp('VALIDACE METOD IGM A DGM')
disp('-----------------------------')
disp('Pozadovana poloha efektoru')
[x;y;q]
disp('Vystup z IGM (kloubove souradnice)')
IGM(ai,[x;y;q])
disp('Vystup z DGM (poloha koncoveho efektoru)')
DGM(ai, ans)

%% validace spravnosti modelu
simout_porovnani = sim('Geometricky_simulacni_model');
disp('VALIDACE DGM A IGM V SIMULACI S MODELEM MANIPULATORU')
disp('Pozadovana poloha efektoru')
[x,y,q]
disp('Vystup modelu')
